...
// --- IMAGES IMPORT (இங்கே தான் மேஜிக் நடக்குது) ---
// குறிப்பு: உங்க ஃபைல் பெயர் வேற மாதிரி இருந்தா, இங்கே மாத்துங்க!
import rahaImg from './assets/rahaa.jpg'; 
import solarImg from './assets/solar.jpg';
// (profile.jpg மட்டும் public ஃபோல்டர்லேயே இருக்கட்டும்)
      ...
      {!loading && (
        <>
          <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 }} className="fixed w-full z-50 px-6 py-6 flex justify-between items-center backdrop-blur-md bg-rahaa-bg/60 border-b border-rahaa-gold/5">
            <h1 onMouseEnter={textEnter} onMouseLeave={textLeave} className="text-xl md:text-2xl font-luxury font-bold text-rahaa-gold tracking-[0.2em] cursor-pointer">RAGUL DRAVID</h1>
          ...
          <section id="home" className="min-h-screen flex flex-col justify-center items-center text-center px-4 pt-20 relative overflow-hidden">
            <div className="z-10 flex flex-col items-center max-w-5xl">
              <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }} className="relative mb-8 group cursor-none" onMouseEnter={textEnter} onMouseLeave={textLeave}>
              ...
              <h2 onMouseEnter={textEnter} onMouseLeave={textLeave} className="text-5xl md:text-8xl font-luxury text-rahaa-text mb-6 leading-tight">
                <RevealText text="CRAFTING DIGITAL" delay={0.2} /><br /><span className="text-rahaa-gold italic"><RevealText text="PERFECTION." delay={0.8} /></span>
              </h2>
          ...
          <div className="py-10 border-y border-rahaa-gold/10 bg-rahaa-gold/5 overflow-hidden backdrop-blur-sm"><MarqueeText /></div>
          ...
          <section id="philosophy" className="py-32 px-6 md:px-20 relative">
             <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
                   ...
                   <p className="text-rahaa-text/60 mb-6 leading-relaxed">With a background in <b>Mathematics</b> and experience in <b>Financial Operations</b>, I bring a unique analytical approach to development. My code isn't just functional; it's optimized for speed, security, and scalability.</p>
                </motion.div>
          ...
          <footer id="contact" className="py-24 text-center border-t border-rahaa-gold/10 bg-gradient-to-b from-rahaa-bg to-black relative z-10">
             <div className="mb-10">
                ...
                <a href="mailto:raguldravid917@gmail.com" onMouseEnter={textEnter} onMouseLeave={textLeave} className="block mt-6 text-2xl md:text-3xl text-rahaa-gold/80 hover:text-rahaa-gold transition-colors font-luxury">raguldravid917@gmail.com</a>
             </div>
...
const MarqueeText = () => (
  <div className="flex overflow-hidden whitespace-nowrap">
      <motion.div animate={{ x: [0, -1000] }} transition={{ repeat: Infinity, duration: 25, ease: "linear" }} className="flex gap-16 opacity-50 font-luxury text-2xl text-rahaa-gold uppercase tracking-widest">
          {[...Array(10)].map((_, i) => (<span key={i} className="flex items-center gap-4">React.js ✦ Node.js ✦ TensorFlow ✦ MongoDB ✦ Razorpay ✦ AWS ✦</span>))}
      </motion.div>
  </div>
);
...
const TiltProjectCard = ({ title, cat, desc, link, align, onMouseEnter, onMouseLeave, image }) => {
  ...
  const handleMouseMove = (event) => { const rect = event.currentTarget.getBoundingClientRect(); x.set(event.clientX - rect.left - rect.width / 2); y.set(event.clientY - rect.top - rect.height / 2); };
  const isRahaa = title.includes("Rahaa");

  return (
      <motion.div style={{ perspective: 1000 }} className={`flex flex-col md:flex-row gap-12 items-center ${align === 'right' ? 'md:flex-row-reverse' : ''}`} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
          <motion.div style={{ rotateX, rotateY }} onMouseMove={handleMouseMove} onMouseLeave={() => { x.set(0); y.set(0); }} className="flex-1 w-full h-[300px] md:h-[400px] relative overflow-hidden group border border-rahaa-gold/20 rounded-2xl cursor-pointer shadow-2xl shadow-black/50 bg-black">
               ...
               <div className="absolute inset-0 flex items-center justify-center overflow-hidden rounded-2xl">
                  ...
                  {!isRahaa && (<div className="absolute inset-0 opacity-20 z-20 pointer-events-none" style={{ backgroundImage: 'linear-gradient(rgba(0, 255, 255, 0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(0, 255, 255, 0.1) 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>)}
               </div>
               ...
               <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-500 z-40"><a href={link} target="_blank" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className="w-24 h-24 bg-rahaa-gold/90 rounded-full flex items-center justify-center text-rahaa-bg font-bold text-xs tracking-widest uppercase shadow-[0_0_30px_rgba(229,184,110,0.6)] transform translate-y-10 group-hover:translate-y-0 transition-all duration-500 hover:scale-110">Visit</a></div>
          </motion.div>
          <div className="flex-1 space-y-6 text-center md:text-left"><span className="text-rahaa-gold text-xs tracking-[0.3em] uppercase border-b border-rahaa-gold/20 pb-2">{cat}</span><h4 className="text-4xl md:text-5xl font-luxury text-white group-hover:text-rahaa-gold transition duration-300">{title}</h4><p className="text-rahaa-text/70 leading-relaxed max-w-md text-sm md:text-base font-light">{desc}</p><a href={link} target="_blank" className="inline-block mt-4 text-xs font-bold text-rahaa-gold uppercase tracking-widest hover:text-white transition">See Live Project →</a></div>
      </motion.div>
  );
};

const RevealText = ({ text, delay }) => (<motion.span initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.05, delayChildren: delay } } }}>{text.split("").map((char, index) => (<motion.span key={index} variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="inline-block">{char === " " ? "\u00A0" : char}</motion.span>))}</motion.span>);
const SectionHeader = ({ title, num }) => (<div className="flex justify-between items-end mb-16 border-b border-rahaa-gold/20 pb-6"><h3 className="text-4xl md:text-5xl font-luxury text-rahaa-text">{title}</h3><span className="text-rahaa-gold font-luxury text-xl italic">{num}</span></div>);
const StatBox = ({ num, label, desc }) => (<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="p-6 border border-rahaa-gold/10 bg-rahaa-bg hover:border-rahaa-gold/40 transition duration-300 group rounded-xl"><h4 className="text-3xl font-luxury text-rahaa-gold mb-1">{num}</h4><p className="text-white text-xs font-bold uppercase tracking-wider mb-2">{label}</p><p className="text-rahaa-text/50 text-[10px]">{desc}</p></motion.div>);
const GridCard = ({ title, skills, delay, icon }) => (<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -10, borderColor: "rgba(229,184,110, 0.5)" }} transition={{ delay, duration: 0.6 }} viewport={{ once: true }} className="bg-rahaa-bg p-10 border border-rahaa-gold/10 transition-all duration-300 group hover:shadow-[0_0_20px_rgba(229,184,110,0.1)] rounded-xl"><div className="mb-6 text-rahaa-gold/50 group-hover:text-rahaa-gold transition duration-500">{icon}</div><h4 className="text-xl font-luxury text-rahaa-gold mb-6 group-hover:translate-x-2 transition duration-300">{title}</h4><ul className="space-y-3">{skills.map(s => <li key={s} className="text-rahaa-text/60 text-sm tracking-wide group-hover:text-rahaa-text transition flex items-center gap-2"><span className="w-1 h-1 bg-rahaa-gold rounded-full"></span> {s}</li>)}</ul></motion.div>);
const LuxuryButton = ({ text, href, outline, onMouseEnter, onMouseLeave }) => (<a href={href} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className={`px-8 py-4 uppercase tracking-[0.2em] text-xs font-bold transition-all duration-500 relative overflow-hidden group rounded-full ${outline ? 'border border-rahaa-gold text-rahaa-gold hover:text-rahaa-bg' : 'bg-rahaa-gold text-rahaa-bg hover:text-white'}`}><span className="relative z-10">{text}</span><div className={`absolute inset-0 ${outline ? 'bg-rahaa-gold' : 'bg-black'} transform scale-x-0 origin-left group-hover:scale-x-100 transition-transform duration-500 ease-out`} /></a>);
const SocialLink = ({ href, text, onMouseEnter, onMouseLeave }) => (<a href={href} target="_blank" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className="text-rahaa-text/50 hover:text-rahaa-gold transition text-xs tracking-widest uppercase border-b border-transparent hover:border-rahaa-gold pb-1 flex items-center gap-2">{text}</a>);